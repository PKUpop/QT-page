...
Qt在不同的操作系统上依赖于不同的运行环境。在完成依赖环境的配置后，Qt安装程序在不同系统上的安装过程是一致的。
...
不断选择下一步，直到进入安装文件夹选择界面，其中的安装选项直接选择“Custom installation”即可。

![安装选项](image/install-select.png)

##### 2.3.4 选择组件

下一步的选择组件界面，按照图中所示勾选需要安装的组件。其中，Qt 6.5.0可以换成别的版本，一般推荐安装最新版本。

![选择组件](image/install-preview.png)
